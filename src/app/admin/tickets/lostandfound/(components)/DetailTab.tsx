...
import { RemarksType } from '@/modules/reports/data/reportTypes';
...
const DetailTab = ({ report }: { report: LostAndFoundDetailType }) => {
  ...
  return (
    <div className="m-4 flex flex-col">
      ...
      <div className="text-base mt-4">Products</div>
      <div className="flex mt-2 gap-2">
        {report.products?.map((product, index) => {
          return (
            <div className="border rounded-md px-4 py-2" key={index}>
              {product.name}
            </div>
          );
        })}
