...
const ResolutionTab = ({ report }: { report: LostAndFoundDetailType }) => {
  ...
  useEffect(() => {
    dispatch(
      lostandfoundApi.endpoints.getLostAndFoundResolutionsList.initiate({
        page: pageIndex + 1,
  ...
  const resolutionsPaginatedData = useAppSelector(
    (state: RootState) =>
      state.baseApi.queries[`getLostAndFoundResolutionsList`]
  ...
  const onSubmit = async (values: ResolutionFormType) => {
    ...
    try {
      dispatch(
        lostandfoundApi.endpoints.addLostAndFoundResolution.initiate({
